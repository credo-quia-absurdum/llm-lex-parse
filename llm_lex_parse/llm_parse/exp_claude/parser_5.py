...
class LRParser:
    ...
    def _parse_expr(self):
        ...
        # Try to parse as assignment: unary '=' expr
        # We need lookahead to detect if this is assignment
        start_pos = self.pos
        
        # First, try to peek ahead to see if we have "ID = ..." pattern
        # This is a simplified check for the most common assignment case
        if self._current().kind == "ID" and self._peek().kind == "=":
            # This is an assignment: unary '=' expr
            self._parse_unary()  # Parse the ID
            self._consume("=")
            self._parse_expr()  # Recursive call for RHS
            self._emit("expr", "unary", "'='", "expr")
        else:
            # Not assignment, parse as binary
            self._parse_binary()
